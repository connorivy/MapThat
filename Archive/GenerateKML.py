...
class KMLifyer():
	...
	def create_pm(self, lat, lon,img_file):
		...
		pm = kml.Placemark('{http://www.opengis.net/kml/2.2}', str(self.count),name, f'<img style="max-width:500px;" src="file:///{img_file}">'.encode(encoding='ascii',errors='strict'))
		pm.geometry = point
	...
	def gen_kml(self):
		...
		kml_string.replace('&gt', '>')
		kml_string.replace('&lt', '<')
		return kml_string
